...
func (s *ColaServer) Allocate(ctx context.Context, reqs *pluginapi.AllocateRequest) (*pluginapi.AllocateResponse, error) {
	...
	for _, req := range reqs.ContainerRequests {
		...
		ds[1] = &pluginapi.DeviceSpec{
			HostPath: "/dev/allegroDecodeIP",
			ContainerPath: "/dev/allegroDecodeIP",
			Permissions: "rwm",
		}
		ds[2] = &pluginapi.DeviceSpec{
			HostPath: "/dev/allegroIP",
			ContainerPath: "/dev/allegroIP",
			Permissions: "rwm",
		}
		...
		mt[1] = &pluginapi.Mount{
			HostPath: "/dev/allegroDecodeIP",
			ContainerPath: "/dev/allegroDecodeIP",
			ReadOnly: false,
		}
		mt[2] = &pluginapi.Mount{
			HostPath: "/dev/allegroIP",
			ContainerPath: "/dev/allegroIP",
			ReadOnly: false,
		}
